...
var SDKServer = class {
  ...
  async createSessionToken(openId, options = {}) {
    return this.signSession(
      {
        openId,
        appId: ENV.appId || "chatforall",
        name: options.name || "Usu\xE1rio"
      },
      options
    );
  }
  async signSession(payload, options = {}) {
    ...
    return new SignJWT({
      ...
      appId: payload.appId || "chatforall",
      name: payload.name || "Usu\xE1rio"
  ...
  async verifySession(cookieValue) {
    ...
    try {
      ...
      return {
        ...
        appId: typeof appId === "string" && appId ? appId : "chatforall",
        name: typeof name === "string" ? name : "Usu\xE1rio"
      };
    } catch (error) {
...
var messagingRouter = router({
  createDirect: protectedProcedure.input(z4.object({ userId: z4.number().int().positive() })).mutation(async ({ ctx, input }) => {
    ...
    // Upsert atômico — evita falha no TiDB com conexões instáveis
    await db.insert(conversations).values({ type: "direct", directKey })
      .onDuplicateKeyUpdate({ set: { type: "direct" } });
    const conv = await db.select({ id: conversations.id }).from(conversations).where(eq4(conversations.directKey, directKey)).limit(1);
    if (!conv[0]) throw new TRPCError5({ code: "INTERNAL_SERVER_ERROR", message: "Erro ao criar conversa." });
