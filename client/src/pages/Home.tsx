...
function NewChatDialog({ open, onOpenChange, onChoose }: { open: boolean; onOpenChange: (value: boolean) => void; onChoose: (conversationId: number) => void }) {
  ...
  const create = trpc.messaging.createDirect.useMutation({
    onSuccess: result => {
      ...
      onChoose(result.conversationId);
      onOpenChange(false);
      setQuery("");
    },
    onError: error => toast.error(error.message),
...
export default function Home() {
  ...
  const chooseConversation = (conversationId: number) => { setSelectedId(conversationId); setSidebarOpen(false); utils.messaging.list.invalidate(); };
